...
interface NLData { date: string; content: string; movers: Mover[]; earningsCount: number; aiGenerated: boolean }

function md2html(text: string) {
  return text
    .replace(/^## (.+)$/gm, '<h2 style="font-family:Barlow Condensed,sans-serif;font-size:26px;font-weight:900;text-transform:uppercase;letter-spacing:1px;color:#111827;margin:0 0 2px">$1</h2>')
...
export default function NewsletterPage() {
  ...
  const load = () => {
    setLoading(true)
    fetch('/api/newsletter/generate').then(r=>r.json()).then(d=>{setData(d);setLoading(false)}).catch(()=>setLoading(false))
  }
  useEffect(() => { load() }, [])
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {data?.aiGenerated && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-200">AI</span>}
            </div>
            <h1 className="text-3xl font-black uppercase text-gray-900" style={{fontFamily:"'Barlow Condensed'"}}>Market Intelligence</h1>
            <p className="text-sm text-gray-500 mt-0.5">{data?.date ? new Date(data.date+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}) : 'Loading...'}</p>
          </div>
          <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shrink-0">
        ...
        {(data?.movers||[]).length > 0 && (
          <div className="flex gap-2 overflow-x-auto pb-2 mb-5">
            {data!.movers.map(m=>(
              <div key={m.symbol} className="flex-shrink-0 px-3 py-2 rounded-xl bg-white border border-gray-200 shadow-sm text-center min-w-[72px]">
                ...
                <div className="text-[11px] text-gray-500 font-mono">undefined</div>
              </div>
            ))}
            {(data?.earningsCount||0)>0 && (
              <div className="flex-shrink-0 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-center min-w-[72px]">
                <Calendar size={14} className="text-amber-600 mx-auto mb-0.5"/>
                <div className="font-black text-lg text-amber-600 leading-none" style={{fontFamily:"'Barlow Condensed'"}}>{data!.earningsCount}</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 mb-5">
          {loading ? (
            <div className="space-y-3 animate-pulse">{[...Array(10)].map((_,i)=><div key={i} className={"h-4 rounded bg-gray-100 "+(i%3===0?'w-1/3':i%2===0?'w-full':'w-4/5')}/>)}</div>
          ) : data?.content ? (
            <div dangerouslySetInnerHTML={{__html: md2html(data.content)}}/>
          ) : (
            <p className="text-center text-gray-400 py-8">Failed to load. Make sure FINNHUB_API_KEY is set in Vercel.</p>
          )}
        </div>
          ...
          {subState==='done' ? (
            ...
            <div className="flex gap-2">
              <input type="email" placeholder="your@email.com" value={email} onChange={e=>setEmail(e.target.value)}
                ...
                className="flex-1 px-3 py-2 text-sm rounded-lg bg-white/10 border border-white/20 text-white placeholder-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-300"/>
              <button onClick={subscribe} disabled={subState==='loading'}
                className="px-5 py-2 text-sm font-bold rounded-lg bg-yellow-400 text-gray-900 hover:bg-yellow-300 transition-colors disabled:opacity-50">
