...
function toHtml(md: string): string {
  return md
    .replace(/^## (.+)$/gm, '<h2 style="font-family:Barlow Condensed,sans-serif;font-size:26px;font-weight:900;text-transform:uppercase;color:#111827;margin:0 0 2px">$1</h2>')
    .replace(/^### (.+)$/gm, '<p style="color:#6b7280;font-size:13px;margin:0 0 20px">$1</p>')
    .replace(/^**(.+)**$/gm, '<h3 style="font-size:13px;font-weight:800;text-transform:uppercase;color:#1e3a5f;margin:20px 0 8px;padding-bottom:6px;border-bottom:2px solid #e5e7eb">$1</h3>')
    ...
    <div className="min-h-screen" style={{ background: '#f7f8fa' }}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-start justify-between mb-6">
          ...
          <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shrink-0 mt-1">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-5">
            ...
            {(brief?.earningsCount || 0) > 0 && (
              <div className="flex-shrink-0 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-center min-w-[70px]">
                ...
                <div className="font-black text-xl text-amber-600 leading-none" style={{ fontFamily: "'Barlow Condensed'" }}>{brief!.earningsCount}</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 mb-5">
          {loading ? (
            ...
            <p className="text-center text-gray-400 py-8">Failed to load. Check FINNHUB_API_KEY in Vercel env vars.</p>
          )}
        </div>
        ...
        <div className="rounded-2xl p-6 border" style={{ background: 'linear-gradient(135deg,#1e3a5f,#2563eb)', borderColor: '#2563eb' }}>
          ...
          {subState === 'done' ? (
            <div className="flex items-center gap-2 text-yellow-300 font-bold text-sm"><Zap size={14} />You are in! Look for tomorrow's brief.</div>
          ) : (
