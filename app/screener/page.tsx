...
const RC: Record<string,string> = { 'Strong Buy':'#3fb950','Buy':'#8bc34a','Hold':'#e3b341','Sell':'#f0883e','Strong Sell':'#f85149' }
const SEC: Record<string,string> = { Technology:'#58a6ff',Financials:'#3fb950',Healthcare:'#a371f7','Consumer Disc.':'#f0883e',Energy:'#d29922',Industrials:'#58a6ff','Consumer Staples':'#e3b341',Communication:'#ec4899' }
  ...
  return (
    <div className="min-h-screen" style={{background:'#080c10'}}>
        ...
        <div className="max-w-screen-2xl mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1 ml-2">
            {([['/', 'Markets'],['/earnings','Earnings'],['/screener','Screener']] as [string,string][]).map(([h,l])=>(
              <Link key={h} href={h} className={`px-3 py-1.5 text-sm rounded transition-colors ${h==='/screener'?'text-white bg-white/8':'text-gray-400 hover:text-white hover:bg-white/5'}`}>{l}</Link>
            ))}
          </nav>
        </div>
      </header>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-black uppercase" style={{fontFamily:"'Barlow Condensed'"}}><Filter size={24} className="inline mr-2 text-green-400"/>Stock Screener</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3 mb-6 p-4 rounded-xl" style={{background:'#0d1117',border:'1px solid #1c2430'}}>
          ...
          <input type="number" placeholder="Min $" value={minPrice} onChange={e=>setMinPrice(e.target.value)} className="px-3 py-2 text-sm rounded-lg focus:outline-none" style={{background:'#161b22',border:'1px solid #30363d',color:'#e6edf3'}}/>
          <input type="number" placeholder="Max $" value={maxPrice} onChange={e=>setMaxPrice(e.target.value)} className="px-3 py-2 text-sm rounded-lg focus:outline-none" style={{background:'#161b22',border:'1px solid #30363d',color:'#e6edf3'}}/>
        </div>
            ...
            <table className="w-full text-sm" style={{minWidth:'900px'}}>
              <thead>
                <tr style={{background:'#0d1117',borderBottom:'1px solid #1c2430'}}>
                  {cols.map(([col,label])=>(
                    <th key={col} onClick={()=>toggleSort(col)} className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-500 cursor-pointer hover:text-white whitespace-nowrap">
              ...
              <tbody>
                {loading ? Array.from({length:8}).map((_,i)=>(
                  <tr key={i} style={{borderBottom:'1px solid #1c2430'}}>
                    {cols.map((_,j)=><td key={j} className="px-4 py-3"><div className="h-4 rounded animate-pulse" style={{background:'#1c2430',width:j===0?'80px':'60px'}}/></td>)}
                  </tr>
                )) : stocks.map(s=>(
                  <tr key={s.symbol} className="hover:bg-[#1c2430]/40 transition-colors cursor-pointer" style={{borderBottom:'1px solid #1c243020'}}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded flex items-center justify-center text-[10px] font-black" style={{background:`${SEC[s.sector]||'#58a6ff'}20`,color:SEC[s.sector]||'#58a6ff'}}>{s.symbol.slice(0,2)}</div>
                        <div>
                          <div className="font-bold" style={{fontFamily:"'IBM Plex Mono'"}}>{s.symbol}</div>
                    ...
                    <td className="px-4 py-3 text-gray-300">{fmtM(s.mktCap)}</td>
                    ...
                    <td className="px-4 py-3"><span className="text-xs font-bold px-2 py-1 rounded" style={{color:RC[s.analystRating]||'#7d8590',background:`${RC[s.analystRating]||'#7d8590'}15`}}>{s.analystRating}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {!loading && stocks.length===0 && <div className="py-16 text-center text-gray-500"><p>No stocks match your filters.</p></div>}
        </div>
        ...
        <div className="mt-8 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4" style={{background:'linear-gradient(135deg,#0d1117,#0f1f0f)',border:'1px solid #2d4a2d'}}>
          <div>
            ...
            <h3 className="text-xl font-black mb-1" style={{fontFamily:"'Barlow Condensed'"}}>Advanced Screener and Alerts</h3>
            <p className="text-sm text-gray-400">50+ filters, options flow, dark pool data, AI stock scoring and real-time alerts.</p>
          </div>
